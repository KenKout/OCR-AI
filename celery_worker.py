...
def process_image_task(image, pdf_id, image_index, task_id, MODE, type_task='LaTex', user_instruction=None, system_instruction=None):
    if MODE == 'COZE':
        while True:
            try:
        ...
        while True:
            ...
            try:
                print("Page: " + str(image_index) + ' - ' + "Sending image data to chat")
                data_return = image_processing_google(image, api_key, type_task, user_instruction, system_instruction)
                if "Error" in data_return:
                    print("Page: " + str(image_index) + ' - ' + "Error in data return")
...
@celery_app.task
def process_task(pdf_path, task_id, type_task, user_instruction=None, system_instruction=None):
    pdf_collection, result_collection, account_collection, google_api_collection = connect()
    ...
    with ThreadPoolExecutor(max_workers=5) as executor:
        for index, image in enumerate(images):
            tasks.append(executor.submit(process_image_task, image, pdf_id, index, task_id, MODE, type_task, user_instruction, system_instruction))
            time.sleep(15)
        for task in tasks:
